...
constrg = {'host': mysqlsrv, 'database': 'autoswitch',
           'user': 'MySQLClient', 'password': '123'}
...
def on_connect(mosq, obj, flags, rc):
    #print("rc: " + str(rc))
    # To Change Global Varible in Local Function variable followed by variable name then assignment
    global ismqttsrvconnected
    ismqttsrvconnected = True
    print("MQTT server is Connected..")


def on_message(mosq, obj, msg):
    print("Current Date-Time : {0:%d-%m-%Y %H:%M:%S} , {1} | {2} | {3}".format(
        datetime.now(), msg.topic, str(msg.qos), str(msg.payload.decode())))
    topic = str(msg.topic)
    ...
    if splitted_topic[1] == "ACK":
        ...
        if splitted_topic[1] == "MS001" or "MSE01":
            mainsstatus = 1 if splitted_msg[2] == 'T' else 0
            update_mainload(splitted_msg[1], mainsstatus)
            update_manual_switch_by_mainload(int(splitted_msg[1]), mainsstatus)
...
def publishing_msg(msg_row, switchstate):
    ...
    mqttc.publish(ptopic, pmessage, 0)
    islogged = False
    discription = "{0} switch is {1} in {2} Board at {3}".format(
        msg_row['SwitchName'], stat, msg_row['BoardName'], msg_row['Location'])
    print(discription, '\n', '-' * len(discription) * 2)
    ...
    try:
        ...
        for row in results:
            ...
            if row['IsMainLoadIndependent'] == 0 and mainloadstatus == 0:
                switchstatus = 0
                print('Switch : {0} is set to OFF due to Main Load failed.'.format(
                    row['SwitchName']))
            if schedule == 0:
...
def update_mainload(main_loadID, main_Status):
    ...
    try:
        dbs = pymysql.connect(**constrg)
        curs = dbs.cursor()
        insqry = "update mainload set MainStatus = {0} where MainLoadID={1}".format(
            main_Status, main_loadID)
        curs.execute(insqry)
        # Commit your changes in the database
        dbs.commit()
    except:
        print("Error: unable to update MainLoad Status")
        # Rollback in case there is any error
        dbs.rollback()
...
def update_manual_switch_by_mainload(mainload_ID, mainload_state):
    ...
    sql = "SELECT * FROM scheduler where IsScheduled = 0 and IsMainLoadIndependent = 0 and MainLdID = {0}".format(
        mainload_ID)
    try:
        dbs = pymysql.connect(**constrg)
        dbs.cursorclass = pymysql.cursors.DictCursor
        curs = dbs.cursor()
        # Execute the SQL command
        curs.execute(sql)
        # Fetch all the rows in a list of lists.
        ...
        print("Error: unable to update Switch Status as MainLoad Status")
    finally:
        curs.close()
        dbs.close()
...
mqttc.subscribe("ND/MS001/#", 0)
mqttc.subscribe("ND/MSE01/#", 0)
# Publish a message
