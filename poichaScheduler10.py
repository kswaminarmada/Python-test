...
# ===================  Prepare SQL query to READ all record one by one FROM the database. ==========
def read_recordNexecute():
      ...
      #print(snd)
      #print(end)
      nowInSec = int(nowInSec[:fst])*3600 +int(nowInSec[fst+1:snd])*60+int(nowInSec[snd+1:end])
         ...
         while results is not None :
             for row in results:
                ...
                if  ondatetime != None and ondatetime < datetime.now():
                    ...
                    query = "UPDATE scheduler SET ondatetime = '0000-00-00 00:00:00' WHERE switchid = '%s'" %(switchid)
                    print (query) 
                    run_query(query)
                ...
                if  offdatetime != None and offdatetime < datetime.now():
                    ...
                    query = "UPDATE scheduler SET offdatetime = '0000-00-00 00:00:00' WHERE switchid = '%s'" %(switchid)
                    print (query) 
                    run_query(query)
                ...
                print(fst)
                print(snd)
                ontimeInSec = int(ontimeInSec[:fst])*3600 +int(ontimeInSec[fst+1:snd])*60+int(ontimeInSec[snd+1:])
                ...
                print(fst)
                print(snd)
                offtimeInSec = int(offtimeInSec[:fst])*3600 +int(offtimeInSec[fst+1:snd])*60+int(offtimeInSec[snd+1:])
...
mqttc.connect("192.168.128.51", 1883)
...
while True :
    ...
    db = pymysql.connect("localhost","root","brainstorm","boarddb" )
